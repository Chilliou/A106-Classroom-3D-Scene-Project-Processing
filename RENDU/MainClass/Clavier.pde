...
private PShape testClavier;
private PShape touche[][]; //22x6
...
PShape objectClavier()
{
  ...
  testClavier = cubeMagiqueClavier((iClavierLargeur/2)+2,iClavierHauteur,iClavierLongueur/2+2,0);
  ...
  for(int i =0;i<iClavierLongueur-1;i++)
  {
    for(int y =0;y<iClavierLargeur-1;y++)
    {
      if(i%2 == 0 && y%2 == 0)
      {
         touche[i/2][y/2] = cubeMagiqueClavier(1,1,1,255);
  ...
  clavierShape.addChild(testClavier);
  testClavier.rotateY(PI/2);
  testClavier.translate(21,0,6);
...
PShape cubeMagiqueClavier(int tailleX, int tailleY, int tailleZ,int couleur) {
